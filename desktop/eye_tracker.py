...
class EyeTracker:
    ...
    def _process_frame(self, frame, face_mesh) -> Optional[Dict]:
        ...
        return {
            ...
            'session_duration': (current_time - self.session_start).total_seconds() if self.session_start else 0,
            'last_blink_time': self.last_blink_time.isoformat() if self.last_blink_time else None
        }
    ...
    try:
        if tracker.start_tracking():
            ...
            while tracker.is_running:
                ...
                print(f"Current stats: {stats['blink_count']} blinks in {stats['session_duration']:.1f}s")
        else:
