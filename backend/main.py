...
from backend.models import init_database, User, BlinkSample, UserSession, TrackingSession, EmailOTP, UserPassword
...
RATE_LIMIT_MAX_ATTEMPTS = int(os.getenv("LOGIN_RATE_LIMIT_MAX", "10"))    # attempts per (email, ip)
RATE_LIMIT_BLOCK_SECONDS = int(os.getenv("LOGIN_BLOCK_SECONDS", "300"))   # block duration after exceeding
...
def check_login_rate_limit(email: str, request: Request):
    ...
    if len(dq) >= RATE_LIMIT_MAX_ATTEMPTS:
        ...
        raise HTTPException(status_code=429, detail=f"Too many attempts. Retry in {RATE_LIMIT_BLOCK_SECONDS}s")
...
def startup_checks():
    ...
    if os.getenv("DISABLE_STARTUP_DB_CHECK", "false").lower() in ("1", "true", "yes"):
        logger.warning("Startup DB connectivity check disabled via DISABLE_STARTUP_DB_CHECK")
        READINESS_OK = True
        return
    try:
    ...
    if os.getenv("ADMIN_SHARED_PASSWORD"):
        logger.warning("Using shared admin password model; replace with per-admin hashed passwords for production hardening.")
...
JWT_EXPIRES_MIN = int(os.getenv("JWT_ACCESS_TOKEN_EXPIRE_MINUTES", os.getenv("JWT_EXPIRES_MIN", "10080")))  # default 7 days; allow fallback env
...
def create_access_token(*, user_id: int, email: str, expires_minutes: int = JWT_EXPIRES_MIN, scope: str = "user") -> str:
    now = datetime.utcnow()
    payload = {
        "sub": str(user_id),
        "email": email,
    ...
    if os.getenv("AUTH_DEBUG"):
        logger.info(
            ...
            payload['sub'], scope, payload['iat'], payload['exp'], token[:16]
        )
...
def decode_access_token(token: str) -> Optional[dict]:
    try:
            ...
            if exp + 300 < now_ts:  # expired more than 5 minutes ago
                if os.getenv("AUTH_DEBUG"):
                    logger.warning(
                        ...
                        exp, now_ts, exp - now_ts, token[:16]
                    )
                return None
            if os.getenv("AUTH_DEBUG"):
                logger.info(
        ...
        if os.getenv("AUTH_DEBUG"):
            logger.warning("AUTH_DEBUG: JWT decode exception (%s) prefix=%s", e, token[:16])
        return None

if os.getenv("AUTH_DEBUG"):
    logger.info("AUTH_DEBUG: startup jwt_alg=%s secret_hash=%s expires_min=%s", JWT_ALG, hash(JWT_SECRET), JWT_EXPIRES_MIN)
    ...
    async def auth_debug_mw(request: Request, call_next):
        ...
        if auth and auth.lower().startswith("bearer "):
            ...
            logger.info("AUTH_DEBUG: inbound bearer len=%s prefix=%s", len(token_part), token_part[:24])
...
def get_current_user(credentials: HTTPAuthorizationCredentials = Depends(security), db: Session = Depends(get_db)):
    ...
    if payload and payload.get("sub"):
        ...
        if not user:
            if os.getenv("AUTH_DEBUG"):
                logger.warning("AUTH_DEBUG: JWT sub %s not found in DB", payload.get("sub"))
            raise HTTPException(status_code=404, detail="User not found")
        if os.getenv("AUTH_DEBUG"):
            logger.info(
                "AUTH_DEBUG: token ok for user_id=%s scope=%s exp=%s now=%s", 
                user.id, payload.get("scope"), payload.get("exp"), int(datetime.utcnow().timestamp())
            )
        return user
    ...
    if os.getenv("AUTH_DEBUG"):
        logger.warning(
            "AUTH_DEBUG: auth failure. jwt_decoded=%s legacy_session_found=%s prefix=%s", bool(payload), bool(session_record), token[:10]
        )
    raise HTTPException(
...
async def login(login_data: LoginRequest, request: Request, db: Session = Depends(get_db)):
    ...
    if not user:
        ...
        allow_auto = os.getenv("ALLOW_AUTO_USER_CREATE", "false").lower() in ("1","true","yes")
        if not allow_auto:
...
async def login_password(login_data: LoginRequest, request: Request, db: Session = Depends(get_db)):
    ...
    if user.email.lower() in admin_emails:
        ...
        if not pw:
            raise HTTPException(status_code=400, detail="Password not set for this account. Use OTP or set a password.")
    ...
    jwt_token = create_access_token(user_id=user.id, email=user.email, scope=scope)
    legacy_token = secrets.token_urlsafe(32)
    session = UserSession(user_id=user.id, session_token=legacy_token, expires_at=datetime.utcnow() + timedelta(days=7))
    db.add(session)
    db.commit()
    if os.getenv("AUTH_DEBUG"):
        logger.info("AUTH_DEBUG: login-password issued session user_id=%s legacy_prefix=%s", user.id, legacy_token[:12])
    return LoginResponse(access_token=jwt_token, user=UserResponse.from_orm(user))

@app.post("/v1/auth/set-password")
async def set_password(req: SetPasswordRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    existing = db.query(UserPassword).filter(UserPassword.user_id == current_user.id).first()
...
async def verify_otp(req: VerifyOtpRequest, db: Session = Depends(get_db)):
    ...
    if req.password:
        ...
        if existing_pw and not created_new:
            ...
            raise HTTPException(status_code=400, detail="Password already set. Use password login or reset flow.")
...
async def google_login(google_req: GoogleLoginRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if os.getenv("GOOGLE_LOG_IDINFO"):
            ...
            safe_keys = {k: idinfo.get(k) for k in ["aud", "iss", "sub", "email", "email_verified", "exp", "iat"] if k in idinfo}
...
async def upload_tracking_sessions(
    ...
    for s in batch.sessions:
        ...
        if existing:
            ...
            created += 1
    db.commit()
    return {"message": "Processed tracking sessions", "created": created, "updated": updated}
...
async def get_blink_summary(
    ...
    session_q = db.query(TrackingSession).filter(TrackingSession.user_id == current_user.id)
    if start_date:
    ...
    total_blinks = (sample_summary.total_blinks or 0) or (session_summary.total_blinks or 0) or 0
    ...
    if sample_count > 0:
        ...
        if session_count > 0:
            avg_per_sample = round((session_summary.total_blinks or 0) / session_count, 2)
        else:
            avg_per_sample = 0
...
def require_admin(x_admin_key: Optional[str] = Header(None), authorization: Optional[str] = Header(None)):
...
@app.get("/admin/users")
async def admin_list_users(q: Optional[str] = None, _: None = Depends(require_admin), db: Session = Depends(get_db)):
    query = db.query(User)
    if q:
        ql = f"%{q.lower()}%"
        ...
        if q.isdigit():
            query = query.filter(or_(User.id == int(q), safunc.lower(User.email).like(ql), safunc.lower(User.name).like(ql)))
        else:
            query = query.filter(or_(safunc.lower(User.email).like(ql), safunc.lower(User.name).like(ql)))
    users = query.order_by(User.created_at.desc()).all()
    return [UserResponse.from_orm(u) for u in users]

@app.get("/admin/sessions")
async def admin_list_sessions(limit: int = 500, _: None = Depends(require_admin), db: Session = Depends(get_db)):
    rows = (
        db.query(TrackingSession)
        .order_by(TrackingSession.ended_at_utc.desc())
        .limit(limit)
...
async def admin_get_user_blinks(
    ...
    if end_date:
        query = query.filter(BlinkSample.captured_at_utc <= end_date)
    samples = (
        query.order_by(BlinkSample.captured_at_utc.desc())
...
async def admin_get_user(user_id: int, _: None = Depends(require_admin), db: Session = Depends(get_db)):
...
async def admin_delete_user(user_id: int, _: None = Depends(require_admin), db: Session = Depends(get_db)):
    ...
    admin_emails = {e.strip().lower() for e in os.getenv("ADMIN_EMAILS", "").split(",") if e.strip()}
    if user.email.lower() in admin_emails and os.getenv("ALLOW_ADMIN_ACCOUNT_DELETION", "false").lower() not in ("1","true","yes"):
        raise HTTPException(status_code=400, detail="Deletion of admin account blocked (set ALLOW_ADMIN_ACCOUNT_DELETION=true to override)")
    email = user.email
    blink_deleted = db.query(BlinkSample).filter(BlinkSample.user_id == user_id).delete()
    sess_deleted = db.query(TrackingSession).filter(TrackingSession.user_id == user_id).delete()
    usess_deleted = db.query(UserSession).filter(UserSession.user_id == user_id).delete()
    pwd_deleted = db.query(UserPassword).filter(UserPassword.user_id == user_id).delete()
...
async def admin_delete_user_by_email(email: str, _: None = Depends(require_admin), db: Session = Depends(get_db)):
...
async def delete_me(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    admin_emails = {e.strip().lower() for e in os.getenv("ADMIN_EMAILS", "").split(",") if e.strip()}
    if current_user.email.lower() in admin_emails and os.getenv("ALLOW_ADMIN_ACCOUNT_DELETION", "false").lower() not in ("1","true","yes"):
        raise HTTPException(status_code=400, detail="Admin accounts cannot self-delete (set ALLOW_ADMIN_ACCOUNT_DELETION=true to override)")
    uid = current_user.id
...
try:
    ...
    if next_export_dir.exists():
        app.mount("/dashboard", StaticFiles(directory=str(next_export_dir), html=True), name="dashboard")
except Exception:
